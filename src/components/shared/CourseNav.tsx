...
const CourseNav = ({progress}:Props) => {
  ...
  return (
    <Box sx={{ height: { xs:'10vh',md:'8vh'},borderBottom:'1px dotted #3e4143',display:'flex',justifyContent:'space-between',alignItems:'center',background:'#1c1d1f',color:'#fff',flexDirection:{
      ...
      <Typography variant="h3" sx={{ display: { xs:'none', md: 'block', lg: 'block' }, color: '#fff', fontSize: '35px', fontWeight: '600', fontFamily: 'Roboto sans-serif', letterSpacing: '1px',pr:2, }}>udemy<Box sx={{ display: 'inline', position: 'absolute', left: '0px', top: '0px', fontWeight: 'bold', color: '#a435f0' }}>^</Box></Typography>
      </Link>
        ...
        <Box sx={{display:{xs:'none',md:'flex'},justifyContent:'space-between',alignItems:'center'}}>
           ...
           <Button sx={{border:'1px solid #fff',color:'#fff',fontSize:'11px',px:2,mr:2}}>Share <ReplyIcon sx={{transform:'rotate(-190deg)'}}/></Button>
        </Box>
    </Box>
  )
}
