...
const CourseContent = ({ courses, setProgress }: Props) => {
  ...
  const [lessonOneCompleteCount, setLessonOneCompleteCount] =
    useState<number>(0);
  const [lessonTwoCompleteCount, setLessonTwoCompleteCount] =
    useState<number>(0);
  ...
  const handleProgress = (event: any, id: number, content: CourseInterFace) => {
    ...
    if (event.target.checked) {
      ...
      content.id == "1"
        ? setLessonOneCompleteCount((prev: number) => prev + 1)
        : setLessonTwoCompleteCount((prev: number) => prev + 1);
    }
    if (!event.target.checked) {
      setProgressItem((prev: number) => prev - 1);
      content.id == "1"
  ...
  return (
    <Box sx={{}}>
      ...
      {courses.courseContent.map((content, index) => (
        <Accordion
          key={content.id}
          sx={{ boxShadow: "none", background: "#F7F9FA", cursor: "pointer" }}
        >
          <AccordionSummary
            ...
            <Box>
              ...
              {content.id == "1" && (
                <Typography sx={{ fontSize: "13px" }}>
                  {" "}
                  {content.id == "1"
                    ? lessonOneCompleteCount
                    : lessonTwoCompleteCount}
                  /{content.length} | {content.duration}min{" "}
                </Typography>
              )}
              {content.id == "2" && (
                <Typography sx={{ fontSize: "13px" }}>
                  {" "}
                  {lessonTwoCompleteCount}/{content.length} | {content.duration}
                  min{" "}
                </Typography>
              )}
            </Box>
          ...
          {content.module.map((lesson: Module, index: number) => (
            <AccordionDetails
              ...
              sx={{
                mt: "",
                "&:hover": { background: "#d1d7dc" },
                background: moduleId == lesson.id ? "#d1d7dc" : "",
              }}
            >
              {" "}
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Checkbox
                  value={lesson.id}
                  onChange={() => handleProgress(event, lesson.id, content)}
                  sx={{
                    "&.Mui-checked": {
                      color: "#333",
                    },
                ...
                />
                <Link
                  href={`/courses/${courses.courseName}/learn/lecture/${lesson.id}`}
                >
              ...
              >
                {lesson.video ? (
                  ...
                  >
                    <SlowMotionVideoIcon sx={{ fontSize: "17px", mr: 1 }} />{" "}
                    {lesson.duration}min
                  </Box>
                ) : (
                  <Box
                  ...
                  >
                    <PostAddIcon
                      sx={{ fontSize: "17px", color: "#6a6f73", mr: 1 }}
                    />{" "}
                    {lesson.duration}min
                  </Box>
                )}
