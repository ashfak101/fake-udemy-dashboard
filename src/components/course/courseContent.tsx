 ...
 const [b,setB]=useState<number>(0)
    ...
    const handleProgress=(event:any,id:number,content:CourseInterFace)=>{
        ...
        if(event.target.checked){
          ...
          content.id=='1'?  setC((prev:number)=>prev+1): setB((prev:number)=>prev+1)
                ...
                <Accordion key={content.id} sx={{ boxShadow: 'none', background: '#f2f7f6', cursor:'pointer'}}>
                    ...
                    >
                      ...
                      <Typography sx={{color:'#333',fontWeight:'700',fontSize:'1rem',fontFamily:'Poppins'}}>Section {index + 1} :    {content.title}</Typography>
                       {content.id=='1' && <Typography sx={{fontSize:'13px'}}> {content.id=='1'? c:b}/{content.length} | {content.duration}min </Typography>}
                       {content.id=='2' && <Typography sx={{fontSize:'13px'}}> {b}/{content.length} | {content.duration}min </Typography>}
                    ...
                    </AccordionSummary>
                  {
                      content.module.map((lesson:Module, index:number) => (
                        ...
                        <AccordionDetails  key={lesson.id}  sx={{ mt: "","&:hover":{background:'#d1d7dc'},background:moduleId == lesson.id ? '#d1d7dc':'' }}>   <Box sx={{display:'flex',alignItems:'center'}}> <Checkbox  value={lesson.id} onChange={()=>handleProgress(event,lesson.id,content)}  sx={{ '&.Mui-checked': {
                          color: "#333",
